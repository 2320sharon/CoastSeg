...
def test_auto_that_resolves_to_regions_still_finds_the_region_map(
...
def test_only_the_clipped_layout_looks_for_the_region_map(
...
def test_a_successful_correction_records_the_tide_model(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda *a, **k: pd.DataFrame(
            {
                "dates": pd.to_datetime(["2021-01-15"], utc=True),
                "x": [0.0],
                "y": [0.0],
                "tide": [0.5],
                "transect_id": ["t1"],
